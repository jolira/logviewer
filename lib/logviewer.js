(function (module) {
    ...
    function subscribe(elem, idx) {
        if (idx >= realTime.length) {
            return realTime.push(elem);
        }

        if (realTime[idx] === elem) {
            return;
        }

        return subscribe(elem, idx + 1);
    }
    ...
    function connect(properties, cb) {
        ...
        if (!accessKeyId || !secretAccessKey || !awsAccountId || !bucket) {
